...
- **JVM 应用分析** — JFR 采集 + JVM 特征向量 + GC/safepoint/JIT 瓶颈检测 + 8 条 JVM 调优规则
- **热点 C++ 化** — JVM JIT 热点方法自动识别，Velox 风格向量表达式 + Intel UMF 访存优化，自动生成 C++ 代码并编译为 JNI .so
...
# Java 应用 JFR 采集
arkprobe collect --jfr --jvm-pid <pid> -b jvm_general

# Java 热点 C++ 加速（识别 + 生成 C++ + 编译 + 验证）
...
├── collectors/      # perf / eBPF / system / JFR 数据采集器
├── scenarios/       # 12个预置场景 YAML + 加载器
├── analysis/        # 特征提取 / TopDown / 对比 / 扩展性 / 设计空间 / 平台优化
├── hotspot/         # Java 热点 C++ 化
│   ├── profiler/       # JFR 热点采集 + 字节码提取
│   ├── analyzer/       # 模式匹配（向量表达式/字符串/数学/访存）
│   ├── codegen/        # C++ 代码生成 + Jinja2 模板
│   │   └── templates/  # umf/vector_expr/math/string/generic 模板
│   ├── compiler/       # GCC/Clang 编译 .so
│   └── runtime/         # JNI 加载 + 性能基准测试
├── tuner/           # 硬件调节 + gem5 仿真
...
**New Feature: Java 热点 C++ 化（Velox 风格 + Intel UMF）**

参考 Meta Velox 向量引擎和 Intel Unified Memory Framework，实现 JVM JIT 热点方法自动识别与 C++ 加速：

**热点识别**
- JFR `jdk.ExecutionSample` + `jdk.ProfiledMethod` 事件采集
- 字节码提取（javap / HSDB）
- 模式分类：向量表达式 / 字符串处理 / 数学运算 / 内存带宽

**C++ 代码生成**
- `arkprobe/hotspot/codegen/templates/` 下的 Jinja2 模板
- **Velox 风格模板**：vector_map / vector_reduce / vector_filter（NEON SIMD）
- **Intel UMF 模板**：array_copy / array_scale / array_add / matrix_mul（非访存优化）
- **Math 模板**：sigmoid / relu / gemm / sin / cos / sqrt（libm/SLEEF）
- **String 模板**：split / replace / parse / search（RE2/SIMD）

**编译工具链**
- GCC/Clang 编译 .so，CMake 构建
- JNI RegisterNatives 封装
- BenchmarkRunner：Java vs C++ 性能对比

**新增模块**
- `arkprobe/hotspot/` — 完整的热点加速模块
  - `profiler/` — JFR 热点采集 + 字节码提取
  - `analyzer/pattern_matcher.py` — 模式识别 + subtype 推断
  - `codegen/` — C++ 代码生成 + 5 个模板
  - `compiler/` — 编译工具链
  - `runtime/` — JNI 加载 + 性能基准测试
  - `accelerator.py` — 端到端流水线编排

**Bug 修复（14 项）**
- CRITICAL: JNI 名称修饰双重 Java_ 前缀 + 完整 JNI 规范实现
- CRITICAL: string.cpp.j2 空 target 无限循环 + NewStringUTF OOM 空指针
- CRITICAL: RegisterNatives 失败未返回 JNI_ERR
- HIGH: pattern_matcher SIMD 伪代码修复（ARM NEON → JVM 字节码模式）
- HIGH: jni_loader.py 异常吞没 + JDK8 agent 加载始终返回 True
- HIGH: benchmark_runner.py 纯 sleep 占位 → jcmd JFR 尝试
- MEDIUM: target_arch 默认值跨平台修复、CMake stdc++ 移除、模板 NoneType 崩溃、未使用 import 清理

**集成测试**
- 37 个 hotspot 测试用例（mock JFR/编译器）
- 270 个全量测试通过

### v0.3.0 (2026-04-22)

**New Feature: JVM 应用分析**

- `JfrCollector`：JDK 11+ JFR 采集 + JDK 8 jstat/jstack 回退
- `JvmCharacteristics`：GCMetrics / JITMetrics / JVMThreadMetrics
- `FeatureExtractor._extract_jvm()`：JFR/jstat 数据 → JVM 特征向量
- `BottleneckAnalyzer._analyze_jvm_bottlenecks()`：GC/Safepoint/JIT/Heap 4 类瓶颈检测
- 8 条 JVM 调优规则：heap_max_size, gc_algorithm, gc_thread_count, young_gen_size, metaspace_size, jit_compiler_threads, thread_stack_size, large_pages
- `jvm_general` 内置场景
- `arkprobe collect --jfr --jvm-pid <pid> -b jvm_general`
